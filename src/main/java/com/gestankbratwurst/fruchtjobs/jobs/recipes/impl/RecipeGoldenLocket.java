 ...
 *
 */
public class RecipeGoldenLocket extends AbstractJobShapedRecipe {

  public RecipeGoldenLocket(JobManager jobManager) {
    super(jobManager, JobType.CRAFTER, 0, JobPerkType.GOLD_SMITH);
    shape = new String[]{"GGG", "NGN", " N "};
    desc = new ArrayList<>();
    desc.add("§7Schmuck und Handelsware.");
    desc.add("§7Kann verzaubert werden.");
  ...
  public ItemStack getResult() {
    return ItemLibrary.GOLDEN_LOCKET.getItem();
  }

  @Override
  public void onCraft(Player player, int amount) {
    jobManager.getOnlineHolder(player).addExp(JobType.CRAFTER, amount * 70);
  }
